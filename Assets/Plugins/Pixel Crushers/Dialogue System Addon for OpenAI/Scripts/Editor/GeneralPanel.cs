...
namespace PixelCrushers.DialogueSystem.OpenAIAddon
{
    ...
    public class GeneralPanel : BasePanel
    {
        ...
        public override void Draw()
        {
            ...
            //--- Pricing varies, and user can look it up online.
            //if (GUILayout.Button("OpenAI Pricing", labelHyperlinkStyle))
            //{
            //    Application.OpenURL("https://openai.com/api/pricing/");
            //}
            EditorGUILayout.LabelField("You can also use locally-installed LLMs via Ollama instead of OpenAI. " +
                "Click Fine-Tuned & Custom Models to configure Ollama.", labelWordWrappedStyle);
            ...
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("Fine-Tuned & Custom Models", GUILayout.Width(200)))
            {
                DialogueSystemOpenAIWindow.Open(AIRequestType.FineTunedModels, database, asset, entry, field);
            }
            EditorGUILayout.LabelField("Add your fine-tuned & custom models.");
            EditorGUILayout.EndHorizontal();
