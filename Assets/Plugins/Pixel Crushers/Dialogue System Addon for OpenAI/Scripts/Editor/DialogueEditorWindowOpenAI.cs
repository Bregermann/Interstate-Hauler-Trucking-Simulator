...
namespace PixelCrushers.DialogueSystem.DialogueEditor
{
    ...
    public partial class DialogueEditorWindow
    {
        ...
        {
            if (GUILayout.Button(new GUIContent("AI", "Revise text or translate localization fields."), EditorStyles.toolbarPopup, GUILayout.Width(36)))
            {
                var menu = new GenericMenu();
                menu.AddItem(new GUIContent("Revise", "Revise text"), false,
                    () => { DialogueSystemOpenAIWindow.Open(AIRequestType.ReviseText, database, asset, entry, field); });
                menu.AddItem(new GUIContent("Translate", "Translate text"), false,
                    () => 
                    {
                        FindLanguagesForLocalizationExportImport();
                        DialogueSystemOpenAIWindow.Open(AIRequestType.LocalizeEntry, database, null, entry, field);
                        DialogueSystemOpenAIWindow.Instance.SetLocalizationLanguages(localizationLanguages.languages);
                    });
                menu.ShowAsContext();
            }
        ...
        public void DrawAIVoiceSelection(Actor actor)
        {
            ...
            if (GUILayout.Button(new GUIContent("Select", "Assign an AI voice actor."), EditorStyles.miniButtonRight, GUILayout.Width(size.x)))
            {
        ...
        {
            if (GUILayout.Button(new GUIContent("AI", "Generate text to speech using the selected AI voice actor."), EditorStyles.miniButtonRight, GUILayout.Width(21)))
            {
                DialogueSystemOpenAIWindow.Open(AIRequestType.GenerateVoice, database, null, entry, field);
            }
        }
