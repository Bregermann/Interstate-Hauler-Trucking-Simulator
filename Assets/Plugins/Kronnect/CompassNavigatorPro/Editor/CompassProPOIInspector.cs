        ...
        public override void OnInspectorGUI() {
            ...
            if (expandGeneralSettings) {
                ...
                if (GUILayout.Button("Clear", GUILayout.Width(80))) {
                    Undo.RecordObjects(pois, "Clear POI Id");
                ...
                if (GUILayout.Button("Randomize", GUILayout.Width(80))) {
                    Undo.RecordObjects(pois, "Randomize POI Id");
                    foreach (var poi in pois) {
                        poi.GenerateNewId();
                        EditorUtility.SetDirty(poi);
                    }
                ...
                bool anyZeroId = false;
                for (int i = 0; i < pois.Length; i++) {
                    if (pois[i] != null && pois[i].id == 0) { anyZeroId = true; break; }
                }
                if (anyZeroId) {
                ...
                bool g3 = (mask & 4) != 0;
                bool g4 = (mask & 8) != 0;
                EditorGUI.showMixedValue = compassGroupMask.hasMultipleDifferentValues;
