...
namespace CompassNavigatorPro {
    ...
    [HelpURL("https://kronnect.com/docs/compass-4/")]
    ...
    public partial class CompassProPOI : MonoBehaviour {
        ...
        [Tooltip("Stable, scene-persistent identifier (GUID). Auto-assigned when empty. Used as the key by the Save/Load State API so visited state, focus and runtime POIs survive a reload. Leave empty to let the system assign one.")]
        [SerializeField]
        string stableId;

        /// <summary>
        /// Stable, scene-persistent identifier used by the Save/Load State API. Auto-assigned when empty.
        /// Set this yourself to deterministically link a runtime-created POI to your own world objects.
        /// </summary>
        ...
        /// <summary>
        /// Returns the key used by the Save/Load State API: the stableId, or a fallback derived from the legacy int id for legacy POIs.
        /// </summary>
        public string GetSaveKey () {
            if (!string.IsNullOrEmpty(stableId)) return stableId;
            return "id:" + id;
        }

        public void GenerateNewStableId () {
            stableId = Guid.NewGuid().ToString("N");
        }

        [System.NonSerialized]
        [Tooltip("True when this POI is auto-managed by the route system as the next-waypoint cue. Use it to filter route POIs from your own logic.")]
        public bool isRouteWaypoint;
        ...
        void OnEnable() {
            ...
            if (string.IsNullOrEmpty(stableId)) {
                ...
                // Regenerate a duplicated stableId (editor duplication / prefab-baked id) so save keys stay unique
                CompassPro compassForStableId = CompassPro.instance;
                if (compassForStableId != null) {
                    int stableCount = compassForStableId.pois.Count;
                    for (int k = 0; k < stableCount; k++) {
                        CompassProPOI other = compassForStableId.pois[k];
                        if (other != null && other != this && other.stableId == stableId) {
                            GenerateNewStableId();
#if UNITY_EDITOR
                            if (!Application.isPlaying) UnityEditor.EditorUtility.SetDirty(this);
#endif
