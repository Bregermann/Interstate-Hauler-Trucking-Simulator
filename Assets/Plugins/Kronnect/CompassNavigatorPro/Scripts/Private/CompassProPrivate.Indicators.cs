...
namespace CompassNavigatorPro {
    ...
    public partial class CompassPro : MonoBehaviour {
        ...
        protected virtual void UpdateIndicators() {
            ...
            float onScreenDistanceFactor = DistanceRoundFactor(_onScreenIndicatorShowDistanceFormat);
            float offScreenDistanceFactor = DistanceRoundFactor(_offScreenIndicatorShowDistanceFormat);
            ...
            for (int k = 0; k < poiCount; k++) {
                ...
                if (isOnScreen) {
                    ...
                    if (poi.onScreenIndicatorShowDistance && _onScreenIndicatorShowDistance) {
                        float distanceKey = Mathf.Round(state.distanceToFollow * onScreenDistanceFactor);
                        if (state.prevIndicatorDistance != distanceKey || state.lastIndicatorDistanceText == null) {
                            state.prevIndicatorDistance = distanceKey;
                    ...
                    if (poi.offScreenIndicatorShowDistance && _offScreenIndicatorShowDistance) {
                        float distanceKey = Mathf.Round(state.distanceToFollow * offScreenDistanceFactor);
                        if (state.prevIndicatorDistance != distanceKey || state.lastIndicatorDistanceText == null) {
                            state.prevIndicatorDistance = distanceKey;
