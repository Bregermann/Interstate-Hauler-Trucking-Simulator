...
namespace CompassNavigatorPro {
    ...
    public partial class CompassPro : MonoBehaviour {
        ...
        private void OnValidate () {
            if (_routeCompassBarSprite == null) {
                _routeCompassBarSprite = Resources.Load<Sprite>("CNPro/Sprites/route-cue");
            }
            ...
            _routeWidth = Mathf.Max(0f, _routeWidth);
            _routeWorldWidth = Mathf.Max(0f, _routeWorldWidth);
            _routeWorldYOffset = Mathf.Max(0f, _routeWorldYOffset);
            _routeWorldSampleStep = Mathf.Max(0f, _routeWorldSampleStep);
            _routeWorldMaxDistance = Mathf.Max(0f, _routeWorldMaxDistance);
        ...
        /// Update bar icons
        /// </summary>
        // Rounding factor (10^decimals) for a .NET numeric format, used to detect when a distance label would actually change before re-formatting it
        protected static float DistanceRoundFactor (string format) {
            if (string.IsNullOrEmpty(format)) return 1f;
            int decimals = 0;
            bool dot = false;
            ...
            switch (decimals) {
                ...
                case 2: return 100f;
                case 3: return 1000f;
                default: return Mathf.Pow(10f, decimals);
            }
        }
        ...
        protected virtual void UpdateCompassBarIcons () {
            ...
            float compassBarDistanceFactor = DistanceRoundFactor(_showDistanceFormat);
            ...
            for (int p = 0; p < iconsCount; p++) {
                ...
                // Global show-POIs toggle (the route cue is governed by its own setting)
                if (!_compassShowPOIs && !poi.isRouteWaypoint) {
                    ToggleCompassBarIconVisibility(poi, state, false);
                    continue;
                }
                    ...
                    if (iconVisible) {
                        ...
                        if (state.compassIconDistanceText != null) {
                            ...
                            if (_showDistance && poi.iconShowDistance && state.distanceToFollow > 0.1f) {
                                ...
                                float distanceKey = Mathf.Round(state.distanceToFollow * compassBarDistanceFactor);
                                if (state.lastCompassIconDistance != distanceKey || state.lastCompassIconDistanceText == null) {
                                    state.lastCompassIconDistance = distanceKey;
