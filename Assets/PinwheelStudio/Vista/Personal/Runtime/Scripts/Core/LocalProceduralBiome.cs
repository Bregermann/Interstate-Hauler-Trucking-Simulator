...
namespace Pinwheel.Vista
{
    ...
    /// </remarks>
    public class LocalProceduralBiome : MonoBehaviour, IProceduralBiome, IBiomeTemplateSpawnCallbackReceiver, ISerializationCallbackReceiver
    {
        ...
        /// </remarks>
        public BiomeDataRequest RequestData(Bounds worldBounds, int heightMapResolution, int textureResolution, BiomeDataMask consumableDataMask = (BiomeDataMask)(~0))
        {
            ...
            if (m_terrainGraph != null)
            {
                CoroutineUtility.StartCoroutine(RequestDataProgressive(request, worldBounds, heightMapResolution, textureResolution, consumableDataMask));
                return request;
            }
            else
            {
        ...
        private IEnumerator RequestDataProgressive(BiomeDataRequest request, Bounds worldBounds, int heightMapResolution, int textureResolution, BiomeDataMask consumableDataMask)
        {
            ...
            // Intersect the user's data mask with the channels the caller can consume, so an output is generated only when the
            // biome produces it and the caller uses it. The caller defaults this to all channels.
            ...
            if (cachedData == null)
            {
                ...
                CoroutineUtility.StartCoroutine(TerrainGraphUtilities.RequestBiomeData(this, cacheDataRequest, graphToExecute, biomeWorldBoundsInt, space, m_baseResolution, m_seed, inputContainer, effectiveDataMask, inputProvider.FillTerrainGraphArguments, EnsureGraphExecutionCache()));
