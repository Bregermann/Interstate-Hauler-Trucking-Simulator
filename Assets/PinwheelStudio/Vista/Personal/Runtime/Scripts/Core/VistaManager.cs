...
namespace Pinwheel.Vista
{
    ...
    public partial class VistaManager : MonoBehaviour
    {
        ...
        /// </summary>
        /// <returns><see langword="true"/> when a manager generation task is tracked and has not yet completed; otherwise, <see langword="false"/>.</returns>
        public static bool HasActiveTask()
        {
            return s_activeGenerateTask != null && !s_activeGenerateTask.isCompleted;
        }
        ...
        private IEnumerator ProcessBiomesProgressive(ProgressiveTask taskHandle, IEnumerable<IBiome> biomes, IEnumerable<ITile> overlappedTiles, ICollection<KeyValuePair<ITile, IBiome>> overlapTests)
        {
            ...
            {                
                ...
                foreach (IBiome b in biomes)
                {
                    ...
                    BiomeDataRequest r = b.RequestData(t.worldBounds, heightMapResolution, textureResolution, consumableDataMask);
                    requests.Add(r);
                    sourceBiomes.Add(b);
                    yield return r;
                }
