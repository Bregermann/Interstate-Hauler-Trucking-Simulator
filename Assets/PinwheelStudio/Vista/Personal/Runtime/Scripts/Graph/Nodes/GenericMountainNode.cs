...
namespace Pinwheel.Vista.Graph
{
    ...
    public class GenericMountainNode : ImageNodeBase, IHasSeed
    {
        ...
        public override void ExecuteImmediate(GraphContext context)
        {
            ...
            NodeLibraryUtilities.NoiseNode.Execute(mountainShapeRt, CreateMountainShapeParams(baseSeed, worldBounds));

            RenderTexture peakVariationRt = context.CreateTemporaryRT(descriptor, TEMP_RT_PEAK_VARIATION_NAME);
            NodeLibraryUtilities.NoiseNode.Execute(peakVariationRt, CreatePeakVariationParams(baseSeed, worldBounds));

            RenderTexture combinedRt = context.CreateTemporaryRT(descriptor, TEMP_RT_COMBINED_NAME);
            NodeLibraryUtilities.CombineNode.Execute(mountainShapeRt, peakVariationRt, Texture2D.whiteTexture, NodeLibraryUtilities.CombineNode.MODE_MUL, combinedRt);

            SlotRef outputRef = new SlotRef(m_id, outputSlot.id);
            RenderTexture targetRt = context.CreateRenderTarget(descriptor, outputRef);
            NodeLibraryUtilities.LevelsNode.Execute(combinedRt, targetRt, 0f, 0.5f, 1f, m_minHeight, m_maxHeight);
