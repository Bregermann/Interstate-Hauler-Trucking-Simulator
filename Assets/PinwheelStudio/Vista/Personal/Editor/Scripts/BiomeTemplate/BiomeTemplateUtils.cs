...
namespace Pinwheel.VistaEditor
{
    ...
    public static class BiomeTemplateUtils
    {
        ...
        // A "<SceneName> Biomes" folder beside the scene asset, created if needed. Parallels the terrain flow's
        // per-scene data folder.
        private static string GetOrCreateDataFolder(VistaManager manager)
        {
            ...
            string folderName = string.Format("{0} Biomes", Path.GetFileNameWithoutExtension(scenePath));
            string fullPath = string.Format("{0}/{1}", sceneFolder, folderName);
            if (!AssetDatabase.IsValidFolder(fullPath))
            {
                AssetDatabase.CreateFolder(sceneFolder, folderName);
            }
            return fullPath;
        }
    }
