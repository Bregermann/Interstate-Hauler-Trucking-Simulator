...
namespace Pinwheel.VistaEditor.Wizard
{
    ...
    public class CreateBiomePage : HubPage
    {
        ...
        private void OnCreateClicked()
        {
            ...
            // Regenerate so the new biome shows on the terrain right away. Skipped when Vista
            // is already busy (its own graph-change hook will pick it up), so the create feedback below is never
            // blocked. This is a wizard-flow choice, so it lives here, not in the create factory.
            if (m_manager != null && !VistaManager.HasActiveTask())
                m_manager.GenerateAll();
