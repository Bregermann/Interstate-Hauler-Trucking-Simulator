    ...
    public class LocalProceduralBiomeInspector : Editor
    {
        ...
        private void SnapToCurrentTile()
        {
            ...
            foreach (ITerrainSystem ts in terrainSystems)
            {
                ...
                Object[] tileObjects = Utilities.FindObjectsNoSortCompat<Component>()
                    .Where(c => c != null && tileComponentType.IsAssignableFrom(c.GetType()))
                    .Cast<Object>()
                    .ToArray();
        ...
        public void DrawAnchors(bool editMode, float alphaMul = 1)
        {
            ...
            else
            {
                ...
                //Register a passive default control so the scene view treats this tool as
                //the nearest control while editing anchors. This prevents Unity's built-in
                //click-to-select/marquee picking from stealing the click (and, with shift
                //held, adding the object under the cursor to the selection).
                ...
                if (Event.current.shift)
                {
                    if (Event.current.type == EventType.MouseDown && Event.current.button == 0)
                    {
                        ...
                        //Consume the event so it doesn't fall through to Unity's default
                        //scene-view object picking (which would also select the object
                        //under the cursor, and add it to the selection because shift is held).
                        Event.current.Use();
                    }
                }
                if (EditorGUI.EndChangeCheck())
