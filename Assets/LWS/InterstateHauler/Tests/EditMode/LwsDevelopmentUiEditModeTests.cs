...
namespace LWS.InterstateHauler.Tests.EditMode
{
    public sealed class LwsDevelopmentUiEditModeTests
    {
        ...
        public void PersistenceMenuSourceDefinesReadableF3SaveLoadFlow()
        {
            ...
            StringAssert.Contains("DefaultVisibleRowHeight = 100f", source);
            StringAssert.Contains("ButtonPreferredHeight = DefaultVisibleRowHeight", source);
            StringAssert.Contains("InfoRowPreferredHeight = 72f", source);
            StringAssert.Contains("MinimumVisibleElementHeight = 60f", source);
            StringAssert.Contains("layout.childControlHeight = true", source);
            StringAssert.Contains("layout.flexibleHeight = 0f", source);
            StringAssert.Contains("FinalizeGeneratedLayout", source);
            StringAssert.Contains("Canvas.ForceUpdateCanvases", source);
            StringAssert.Contains("LayoutRebuilder.ForceRebuildLayoutImmediate", source);
            StringAssert.Contains("RepairCollapsedVisibleElements", source);
            StringAssert.Contains("Persistence UI row collapsed to zero height and was repaired", source);
            StringAssert.Contains("AddRow(\"Autosave\", DefaultVisibleRowHeight)", source);
            StringAssert.Contains("AddRow(\"Recovery Offer\", DefaultVisibleRowHeight)", source);
            StringAssert.Contains("AddRow($\"Slot {slot.slotNumber}\", DefaultVisibleRowHeight)", source);
            StringAssert.Contains("AddRow($\"Profile {profile.profileIndex}\", DefaultVisibleRowHeight)", source);
            StringAssert.Contains("row.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, safeHeight)", source);
            StringAssert.Contains("layout.childForceExpandHeight = false", source);
            StringAssert.DoesNotContain("childControlHeight = false", source);
            StringAssert.DoesNotContain("ButtonPreferredHeight = 60f", source);
            StringAssert.DoesNotContain("AddRow($\"Info {label}\", 42f)", source);
