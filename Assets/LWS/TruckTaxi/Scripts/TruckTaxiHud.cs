...
namespace LWS.TruckTaxi
{
    public sealed class TruckTaxiHud : MonoBehaviour
    {
        ...
        public void Initialize(TruckTaxiBootstrap value)
        {
            ...
            var telemetryBand=Panel(root,"Telemetry background",new Vector2(0,0.89f),new Vector2(.72f,1));
            ...
            requestText = Text(ridePanel,"Requests",new Vector2(0.05f,0.05f),new Vector2(0.95f,0.82f),24);
            reaction = Text(root,"Passenger reaction",new Vector2(0.27f,0.15f),new Vector2(0.72f,0.24f),28);
            modal = Panel(root,"Ride dispatch",new Vector2(0.28f,0.30f),new Vector2(0.7f,0.85f));
            title = Text(modal,"Title",new Vector2(0.06f,0.82f),new Vector2(0.94f,0.98f),38);
        ...
        private void Refresh()
        {
            ...
            {
                float pickupDistance=Vector3.Distance(host.Player.transform.position,s.Pickup.StopPosition);
                float trip=Vector3.Distance(s.Pickup.StopPosition,s.Destination.StopPosition);
                details.text=$"{s.Passenger.passengerName}  /  {s.Passenger.passengerRating:0.0} STARS\n{s.Passenger.personality}\n\nPICKUP  {s.Pickup.locationName} ({pickupDistance:0} m)\nTO  {s.Destination.locationName}\nEST. {trip:0} m  /  {Money(host.Configuration.baseFareCents+(long)(trip*host.Configuration.centsPerMeter))}\nOFFER EXPIRES IN {s.OfferRemaining:0}s";
            }
            ...
            if(active)
            {
                ...
                b.AppendLine(s.HasPassenger ? "ON BOARD" : s.State==TruckTaxiState.PassengerBoarding ? "BOARDING..." : "PICKUP / STOP IN BAY");
            ...
            reaction.text=s.ReactionAge<5 && active ? s.Reaction : "";
        }
