        ...
        public override void GenerateEmitters()
        {
            ...
            for (int s = 0; s < splineContainer.Splines.Count; s++)
            {
                ...
                for (int y = 0; y <= sampleCount; y++)
                {
                    ...
                    float3 forward = splineContainer.transform.TransformDirection(math.normalize(tangent));
                    normal = splineContainer.transform.TransformDirection(normal);
                    
                    if (confines == Confines.Bounds)
                    {
                        //Early out if spline point falls outside of bounds
                        if (bounds.Contains(position) == false) continue;
                    }
                    
                    //Analyze the next point to check if there is a large enough drop
                    float3 nextPosition = splineContainer.Splines[s].EvaluatePosition(t + stride);
                    ...
                    if (delta >= heightDropThreshold)
                    {
                        ...
                        //bi-tangent
                        float3 right = math.normalize(math.cross(forward, normal));
