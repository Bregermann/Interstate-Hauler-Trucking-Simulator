...
namespace sc.modeling.river.runtime
{
    ...
    public class AudioZone : MonoBehaviour
    {
        ...
        {
            if(river && river.GetHashCode() == instance.GetHashCode() && (updateFlags.HasFlag(RiverModeler.ChangeFlags.All) || updateFlags.HasFlag(RiverModeler.ChangeFlags.Spline)))
            {
        ...
        public void Rebuild()
        {
            ...
            for (int s = 0; s < splineContainer.Splines.Count; s++)
            {
                ...
                for (int y = 0; y <= sampleCount; y++)
                {
                    ...
                    {
                        position = position,
                        radius = math.max(minAudibleDistance, (scale.x * river.settings.shape.width) * 0.5f)
                    };
        ...
        void RecreateEmitters()
        {
            ...
            for (int i = 0; i < emitters.Length; i++)
            {
                GameObject obj = new GameObject($"AudioEmitter_{i}", typeof(AudioSource));
                obj.transform.parent = this.transform;
                
                AudioSource emitter = obj.GetComponent<AudioSource>();
