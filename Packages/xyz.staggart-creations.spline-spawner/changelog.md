1.2.1 (July 26th 2026)

Fixed:
- Build blocking errors when one or more Spline Spawner components were missing object containers.
- Terrain layer masks not taking effect if a layer higher in the list was disabled.

Changed:
- Multiple Spline Spawner components on the same GameObject is no longer allowed. They’d both detect rogue objects from eachother and will try to adopt them, leading to a cycle of errors.

1.2.0 (July 15th 2026)

Added:
- New core Terrain Layer Masking feature, allows to include/exclude objects from terrain layers.
- Respawn Triggers, determine what events will trigger a respawn. This may be used for advanced scenarios that require full control.
- Modifier add menu: Presets for Random Y/XYZ Rotation and Scale Variation 20%.
- Snap To Grid modifier, option to use the Scene View grid snapping system.

Changed:
- Script: "RespawningMode" enum was renamed to "SplineChangeTrigger"

1.1.8 (July 7th 2026)

Added:
- Menu items under Window/Spline Spawner to open the demo scene or edit the masking layer names.

Changed:
- Minor code updates for Unity 6.5+;

Fixed:
- Size of prefabs with a LOD Group not just being calculated from LOD0, resulting in larger spacing than expected.
- Compute shader warning on some platforms (otherwise harmless).

1.1.7 (April 16th 2026)

Changed:
- Drop To Collider modifier, minor performance improvement.

Fixed:
- Additional meshes used for LOD0 not contributing to the calculated size of a prefab.
- Drop To Collider modifier, slope limit min/max values not accepting manual input.

1.1.6 (March 4th 2026)

Added:
- Support for usage with prefabs.
- Icons for inspector sections.

Changed:
- Minimum allowed spline length is now 0.25m

Fixed:
- "Create" menu item not placing the newly created spawner in a prefab's hierarchy.
