...
namespace sc.splines.spawner.editor
{
    public static class SplineSpawnerEditor
    {
        ...
        public static SplineSpawner CreateSpawner(GameObject target)
        {
            SplineSpawner component = target.GetComponent<SplineSpawner>();

            if (component) return component;
            
            component = target.gameObject.AddComponent<SplineSpawner>();
        ...
        private static void SpawnOnSpline(MenuCommand cmd)
        {
            Transform transform = (Transform)cmd.context;
            
            GameObject gameObject = new GameObject($"{transform.name} Spline");
            Undo.RegisterCreatedObjectUndo(gameObject, "Created Spline Spawner");
            
            Scene scene = transform.gameObject.scene;
            var prefabStage = PrefabStageUtility.GetCurrentPrefabStage();
            if (prefabStage != null) scene = prefabStage.scene;
            SceneManager.MoveGameObjectToScene(gameObject, scene);
            
            gameObject.transform.SetSiblingIndex(transform.gameObject.transform.GetSiblingIndex() + 1);
        ...
        private static void AddSpawnerToSpline(MenuCommand cmd)
        {
            ...
            SplineSpawner component = CreateSpawner(splineContainer.gameObject);
        ...
        public static SplineContainer AddSplineContainer(GameObject target)
        {
            ...
            splineContainer.Spline = CreateDefaultSpline();
