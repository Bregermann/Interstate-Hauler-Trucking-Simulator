...
namespace Dev.Scripts
{
    ...
    public class SnapToGridModifierDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            SerializedProperty useEditorGridSnapping = property.FindPropertyRelative("useEditorGridSnapping");
            SerializedProperty axis = property.FindPropertyRelative("axis");
            
            EditorGUILayout.Separator();

            using (new EditorGUI.DisabledGroupScope(useEditorGridSnapping.boolValue && EditorSnapSettings.gridSnapEnabled))
            {
                EditorGUILayout.PropertyField(gridSize, new GUIContent("Grid size"));
            }

            EditorGUILayout.PropertyField(useEditorGridSnapping, new GUIContent("Use Editor Grid Snapping"));
            if (useEditorGridSnapping.boolValue && !EditorSnapSettings.gridSnapEnabled)
            {
                EditorGUILayout.HelpBox("Grid Snapping is disabled in the scene view. This will have no effect", MessageType.Warning);
            }
            EditorGUILayout.Separator();
