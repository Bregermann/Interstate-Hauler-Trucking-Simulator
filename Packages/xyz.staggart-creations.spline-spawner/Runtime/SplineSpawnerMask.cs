...
{
    [ExecuteAlways]
    [Icon(SplineSpawner.kPackageRoot + "/Editor/Resources/Icons/spline-spawner-mask-icon-64px.psd")]
    public class SplineSpawnerMask : MonoBehaviour
    {
        ...
        public List<SplineSpawner> GetSpawnersAffectedBy()
        {
            ...
            for (int i = 0; i < spawners.Length; i++)
            {
                ...
                for (int j = 0; j < spawner.splineBounds.Count; j++)
                {
                    NativeBounds splineBounds = spawner.splineBounds[j];
